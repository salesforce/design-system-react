...
const defaultProps = {
	...
	assistiveText: {
		itemDragLabel: 'Press space bar when on an item, to move it within the list. CMD plus left and right arrow keys, to move items between lists.',
...
class DuelingPicklist extends React.Component {
	...
	componentDidUpdate(prevProps, prevState) {
		...
		} else if (ariaLiveContext && selectionChanged(selection, prevState.selection)) {
	...
	getIds() {
		return {
			picklistGroupLabel: `${this.getId('picklistGroupLabel')}-picklist-group-label`,
	...
	handleSelect = (option, selectRange, selectAdditionalItem, handledFromFocus = false) => {
		const {
			selection,
			firstSelected,
			lastSelected,
