...
Lib.extend(Pillbox.prototype, PillboxCore, Events, State, {
	...
	_render () {
		...
		this.elements.pillTemplate = this.elements.group.find('.pill').remove().clone();
	...
	_inputKeyUp (e) {
		if ( e.keyCode === 13 ) {
			this._selectItem( this._getItemAdapter({
				text: this.elements.input.val(),
				id: Math.floor(Math.random() * 10000)// Temporary addition
			}));
			this._clearInput();
		}
	},
	...
	_itemClicked (e) {
		const item = this._getItemAdapter($(e.currentTarget).data('item'));
