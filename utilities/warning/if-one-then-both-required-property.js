...
if (process.env.NODE_ENV !== 'production') {
  ...
  ifOneThenBothRequiredProperty = function ifOneThenBothRequiredProperty(control, props, selectedProps, comment) {
    ...
    if (!hasWarned[control]) {
      ...
      (0, _warning2.default)(bothOrNoneAreSet, "[Design System React] If one of the following props are used, then both of the following properties are required by ".concat(control, ": [").concat(keys.join(), "].").concat(additionalComment));
