import React from 'react';
import ReactDOM from 'react-dom';


import badge from '../../src/react/badge/examples/site-example';

if (document.querySelector('#badge-react-control')) {
	ReactDOM.render(React.createElement(badge), document.querySelector('#badge-react-control'));
}

import button from '../../src/react/button/examples/site-example';

if (document.querySelector('#button-react-control')) {
	ReactDOM.render(React.createElement(button), document.querySelector('#button-react-control'));
}

import buttonGroup from '../../src/react/button-group/examples/site-example';

if (document.querySelector('#button-group-react-control')) {
	ReactDOM.render(React.createElement(buttonGroup), document.querySelector('#button-group-react-control'));
}

import checkbox from '../../src/react/checkbox/examples/site-example';

if (document.querySelector('#checkbox-react-control')) {
	ReactDOM.render(React.createElement(checkbox), document.querySelector('#checkbox-react-control'));
}

import combobox from '../../src/react/combobox/examples/site-example';

if (document.querySelector('#combobox-react-control')) {
	ReactDOM.render(React.createElement(combobox), document.querySelector('#combobox-react-control'));
}

import dataTable from '../../src/react/data-table/examples/site-example';

if (document.querySelector('#data-table-react-control')) {
	ReactDOM.render(React.createElement(dataTable), document.querySelector('#data-table-react-control'));
}

import datepicker from '../../src/react/datepicker/examples/site-example';

if (document.querySelector('#datepicker-react-control')) {
	ReactDOM.render(React.createElement(datepicker), document.querySelector('#datepicker-react-control'));
}

import dropdown from '../../src/react/dropdown/examples/site-example';

if (document.querySelector('#dropdown-react-control')) {
	ReactDOM.render(React.createElement(dropdown), document.querySelector('#dropdown-react-control'));
}

import lookup from '../../src/react/lookup/examples/site-example';

if (document.querySelector('#lookup-react-control')) {
	ReactDOM.render(React.createElement(lookup), document.querySelector('#lookup-react-control'));
}

import modal from '../../src/react/modal/examples/site-example';

if (document.querySelector('#modal-react-control')) {
	ReactDOM.render(React.createElement(modal), document.querySelector('#modal-react-control'));
}
import notification from '../../src/react/notification/examples/site-example';

if (document.querySelector('#notification-react-control')) {
	ReactDOM.render(React.createElement(notification), document.querySelector('#notification-react-control'));
}

import picklist from '../../src/react/picklist/examples/site-example';

if (document.querySelector('#picklist-react-control')) {
	ReactDOM.render(React.createElement(picklist), document.querySelector('#picklist-react-control'));
}
import pills from '../../src/react/pills/examples/site-example';

if (document.querySelector('#pills-react-control')) {
	ReactDOM.render(React.createElement(pills), document.querySelector('#pills-react-control'));
}

import popover from '../../src/react/popover/examples/site-example';

if (document.querySelector('#popover-react-control')) {
	ReactDOM.render(React.createElement(popover), document.querySelector('#popover-react-control'));
}
import radio from '../../src/react/radio/examples/site-example';

if (document.querySelector('#radio-react-control')) {
	ReactDOM.render(React.createElement(radio), document.querySelector('#radio-react-control'));
}
import spinner from '../../src/react/spinner/examples/site-example';

if (document.querySelector('#spinner-react-control')) {
	ReactDOM.render(React.createElement(spinner), document.querySelector('#spinner-react-control'));
}

import tooltip from '../../src/react/tooltip/examples/site-example';

if (document.querySelector('#tooltip-react-control')) {
	ReactDOM.render(React.createElement(tooltip), document.querySelector('#tooltip-react-control'));
}

import tree from '../../src/react/tree/examples/site-example';

if (document.querySelector('#tree-react-control')) {
	ReactDOM.render(React.createElement(tree), document.querySelector('#tree-react-control'));
}
