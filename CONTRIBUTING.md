...
- <a name="use-dom-refs" href="#use-dom-refs">#</a> **Avoid querying the DOM for nodes you do not own.** A `ref` callback is React's [link to a DOM node](https://facebook.github.io/react/docs/refs-and-the-dom.html). It is triggered on render and allows access to the DOM node asynchronously. In most cases within a component, you only need a `ref` for setting focus, but `refs` make _testing great again!_ If you find yourself using `querySelectorAll()` to query parts of this library within your application tests, please stop and submit a contribution that surfaces the DOM node you are needing to test. If you control the contents or children yourself, please use your own `ref` to test, for instance `Popover`'s `body` or a custom `DataTableCell`. Once surfaced, a DOM `ref` becomes part of the public API and will be treated as such. **Only prop changes and moving to an SLDS version that has breaking changes are considered breaking changes. SLDS markup alignment and class changes within the current release cycle are not considered breaking changes in this library.**
